...
HEAD_PARAMS = [
    {'angle' : [60.0*almath.TO_RAD, 25.0*almath.TO_RAD], 'time' : 1.0},
    {'angle' : [0.0, 25.0*almath.TO_RAD], 'time' : 2.0},
    {'angle' : [-60.0*almath.TO_RAD, 25.0*almath.TO_RAD], 'time' : 3.0},
...
def find_distance(x, y, r, camera):
...
def get_balls(image, color, bounds, camera):
	...
	if detected_circles is not None:
		for circle in detected_circles[0, :]:
			...
			position, distance = find_distance(x, y, r, camera)
...
for param in HEAD_PARAMS:
	motionProxy.angleInterpolation(names, param['angle'], param['time'], isAbsolute)
